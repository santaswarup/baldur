 ...
 */
class CassandraLeftOuterJoinRDD[L, R] private[connector](
                                                          left: RDD[L],
  ...
  override type Self = CassandraLeftOuterJoinRDD[L, R]
  ...
  override protected def copy(
    ...
    new CassandraLeftOuterJoinRDD[L, R](
      left = left,
  ...
  override def count(): Long = {
    ...
    val counts =
      new CassandraLeftOuterJoinRDD[L, Long](
        left = left,
        connector = connector,
        keyspaceName = keyspaceName,
        tableName = tableName,
    ...
    counts.map(_._2.getOrElse(0.asInstanceOf[Long])).reduce(_+_)
  }
  ...
  def on(joinColumns: ColumnSelector): CassandraLeftOuterJoinRDD[L, R] = {
    new CassandraLeftOuterJoinRDD[L, R](
      left = left,
      connector = connector,
      keyspaceName = keyspaceName,
  ...
  private def fetchIterator(
    ...
    for (leftSide <- lastIt;
         rightSide <- {
           ...
           if (iterator.hasNext) {
             iterator.map(row => {
               Some(rowReader.read(row, columnNamesArray))
             })
           } else {
             Iterator(None)
           }
