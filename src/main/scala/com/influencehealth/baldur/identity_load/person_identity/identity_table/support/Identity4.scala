...
object Identity4 {
  ...
  def create(personIdentity: PersonIdentity): Iterable[Identity4] = {
    personIdentity.mrids.map { mrid =>
      Identity4(personIdentity.customerId,  personIdentity.zip5.get, support.rootFirstName(personIdentity.firstName, personIdentity.sex).get, support.soundex(personIdentity.lastName.get),
